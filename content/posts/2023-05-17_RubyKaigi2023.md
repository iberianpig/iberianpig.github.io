...
FusumaはLinuxのタッチパッド向けのマルチタッチジェスチャのツール。`gem install fusuma` でジェスチャが効くようになる。 ネィティブでジェスチャ実装されてないLinuxディストロでもほとんどのケースでマルチタッチジェスチャが使えるようになる。
...
<div style="left: 0; width: 100%; height: 0; position: relative; padding-bottom: 56.25%;"><iframe src="https://www.youtube.com/embed/gnwe8DqPDrk?rel=0" style="top: 0; left: 0; width: 100%; height: 100%; position: absolute; border: 0;" allowfullscreen scrolling="no" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share;"></iframe></div>
...
実は親指を使わなくても便利で、よく片手だけホームポジションに残してタッチパッド操作している。  
それと、タッチ->Fを押しっぱなしでドラッグをすると、そのドラッグ中はタッチパッドから手が離れても問題ないように実装している。
そのおかげでこの間はタッチパッドを自由にスクロールしたりスワイプしたりできる！！

![Image from Gyazo](https://i.gyazo.com/b434e5c33712b210ea58650451c21caf.gif)

つまり、このようにドラッグ中に別のワークスペースにスワイプして移動してドロップができる。
これまでのマウスやタッチパッドの制約を超えた操作ができるようになる。
ThumbSenseが出た当時はスワイプジェスチャは無かったので、これはかなり画期的だったと思う。
すごく便利だから、みんなLinuxにしてFusuma使ってみよう。

あとタップしてクリックする機能を使わなくなるので、これをオフにするとキーボード入力中にタッチパッド触って誤操作することが無くなって、めちゃくちゃ快適になる。
完全に副作用なのだけれど、タッチパッドの不満点を一気に解消できていて最高。
...
LibinputのPalm Detectionとの兼ね合いが微妙なのでなんとかしたい気持ちもあるし、あとFusumaのinputプラグインはLibinputをFFI経由で使えるようにしたい、色々やりたいことがある。
